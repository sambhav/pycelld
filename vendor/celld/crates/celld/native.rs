...
pub(crate) fn is_entry(root: &Path, entry: &str) -> bool {
    runtime().is_some_and(|r| r.is_entry(&root.join(entry)))
}
...
pub(crate) fn bundle(root: &Path, entry: &str) -> anyhow::Result<Vec<u8>> {
    let source = runtime().ok_or_else(|| anyhow::anyhow!("native runtime is not registered"))?.bundle(root, Path::new(entry))?;
    compile(&source)?;
    Ok(format!(
        "{}{}",
